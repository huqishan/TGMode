...
/// <summary>
/// 界面权限发现服务，负责从导航配置和 XAML 文件中扫描界面、弹窗和按钮节点。
/// </summary>
public static class UiPermissionDiscoveryService
{
        ...
        ref int elementOrder)
    {
        ...
        if (IsButtonElement(localName) && ShouldIncludeButtonElement(element))
        {
    ...
    private static bool ShouldIncludeButtonElement(XElement element)
    {
        if (IsBackdropButtonElement(element))
        {
            return false;
        }
        ...
        return FirstMeaningfulValue(
                   ...
                   GetDisplayAttribute(element, "Click")) is not null;
    }

    private static bool IsBackdropButtonElement(XElement element)
    {
        string? elementName = GetElementName(element);
        string? style = element.Attribute("Style")?.Value;

        return ContainsBackdropMarker(elementName) ||
               ContainsBackdropMarker(style);
    }
